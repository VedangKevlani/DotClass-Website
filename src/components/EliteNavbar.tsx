    ...
    <>
      <header
        className={clsx(
          ...
          scrolled
            ? "bg-white shadow-lg h-20 border-b border-gray-100"
            : "bg-white/95 backdrop-blur-3xl h-24 border-b border-white/20"
        )}
      >
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between px-10">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <button
              ...
              className="md:hidden w-11 h-11 rounded-xl border-2 border-gray-100 flex items-center justify-center hover:border-[#47BA74] bg-white shadow-md transition"
            >
              {isMobileMenuOpen ? <X size={22} className="text-[#47BA74]" /> : <Menu size={22} className="text-gray-700" />}
            </button>
          </div>
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-[280px] bg-white shadow-[0_0_50px_rgba(0,0,0,0.2)] z-[60] md:hidden flex flex-col pt-32 px-8 pb-10"
            >
                ...
                {MENU_ITEMS.map((item) => (
                  <Link
                    ...
                    className={clsx(
                      ...
                      location.pathname === item.path ? "text-[#47BA74]" : "text-gray-700 hover:text-[#47BA74]"
                    )}
                  >
                    {item.label}
                  </Link>
              ...
              <div className="mt-auto flex flex-col gap-4 border-t border-gray-100 pt-8">
                <Link 
                  to="/profile" 
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex items-center gap-3 p-4 rounded-2xl bg-gray-50 text-gray-700 font-semibold"
                >
                  <UserCircle size={22} className="text-[#47BA74]" />
                  Profile Workspace
                </Link>
                <button 
                  onClick={() => { setIsSearchOpen(true); setIsMobileMenuOpen(false); }}
                  className="flex items-center gap-3 p-4 rounded-2xl border border-gray-100 text-gray-600 hover:bg-gray-50 transition"
                >
                  <Search size={22} className="text-[#47BA74]" />
                  Search Site
                </button>
              </div>
            </motion.div>
